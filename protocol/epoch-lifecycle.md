...
After the trading window ends, the vault enters Unwinding while positions are closed and assets are brought back into the vault. During this phase, final execution and settlement are still in progress, so vault balances and share value may continue to update.\
\
At this point you can:

* Wait — no user action is required while the strategy exits and reconciles
* Monitor status — track progress until the vault transitions to Withdrawals Open for redemptions and rollover options

***

## Phase 4 — Withdrawals Open
